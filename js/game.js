...
export class ZeroDivisionGame{
  constructor(){
    ...
    this.keys={};
    this.canJump=false;
    this.dash=false;this.crouch=false;this.slideTimer=0;this.slideVelocity=new THREE.Vector3();this.lean=0;this.ads=false;this.adsKey=false;this.adsMouse=false;this.eyeY=1.72;this.visualBob=0;
  ...
  onMouseDown(e){
    ...
    if(e.button===2){
      if(this.controls.isLocked){
        this.adsMouse=true;this.ads=true;this.updateHUD();
      }
      return;
    }
  ...
  onMouseUp(e){
    ...
    if(e.button===2){
      this.adsMouse=false;this.ads=this.adsKey;this.updateWeaponAnimation(0,false);this.updateHUD();
    }
  }
  ...
  onKey(e,down){
    ...
    // Keyboard look / ADS / fire bindings. Arrow keys control the view while
    // WASD remains dedicated to movement. Period toggles ADS, comma fires while held.
    if((e.code==='ArrowUp'||e.code==='ArrowDown'||e.code==='ArrowLeft'||e.code==='ArrowRight')&&this.running){
      e.preventDefault();
    }
    if(e.code==='Period'&&down&&!e.repeat&&this.running&&!this.paused&&this.controls.isLocked){
      this.adsKey=!this.adsKey;
      this.ads=this.adsKey||this.adsMouse;
      this.updateWeaponAnimation(0,false);
      this.updateHUD();
      e.preventDefault();
    }
    if(e.code==='Comma'&&this.running&&!this.paused&&this.controls.isLocked){
      this.firing=down;
      if(down&&!e.repeat)this.fire();
      e.preventDefault();
    }
  ...
  buildScene(){
    ...
    this.camera.rotation.order='YXZ';this.camera.rotation.set(0,0,0);this.controls.resetView();this.recoilBloom=0;this.ads=false;this.adsKey=false;this.adsMouse=false;this.updateCameraTransform(0);
  ...
  update(dt){
    ...
    // Arrow keys are a second, independent way to look around.
    this.controls.updateKeyboardLook(dt);
  ...
  updateWeaponAnimation(dt,moving){
    ...
    // Subtle walking bob plus a stronger, irregular sprint sway to simulate
    // carrying the weapon while running. This is camera-local and only applies
    // outside ADS/reload, so aiming remains stable.
    const bob=moving?Math.sin(t*(this.dash?14:10))*(this.dash?.012:.006):0;
    this.weaponGroup.position.y+=bob;
    if(this.dash&&moving&&!this.ads&&!this.reloading&&this.sprintBlend>.01){
      const phase=t*17.5;
      const swayX=Math.sin(phase)*.018 + Math.sin(phase*0.53+1.2)*.009;
      const swayY=Math.cos(phase*0.92)*.011;
      const swayZ=Math.sin(phase*0.67+0.8)*.007;
      this.weaponGroup.position.x+=swayX*this.sprintBlend;
      this.weaponGroup.position.y+=swayY*this.sprintBlend;
      this.weaponGroup.position.z+=swayZ*this.sprintBlend;
      this.weaponGroup.rotateZ((Math.sin(phase)*.065 + Math.sin(phase*.47)*.025)*this.sprintBlend);
      this.weaponGroup.rotateX((Math.cos(phase*.88)*.035)*this.sprintBlend);
    }
...
class SimplePointerLock extends EventTarget{
  constructor(camera,domElement){
    super();
    this.camera=camera;this.domElement=domElement;this.isLocked=false;this.pointerSpeed=1;
    this.yaw=0;this.pitch=0;this.recoilYaw=0;this.recoilPitch=0;
    this.keyboardLookSpeed=2.25; // radians/sec at sensitivity 1
    this.keyboardLook={up:false,down:false,left:false,right:false};
    this._onMove=e=>this._move(e);
    this._onKey=e=>{
      const map={ArrowUp:'up',ArrowDown:'down',ArrowLeft:'left',ArrowRight:'right'};
      const key=map[e.code];
      if(!key)return;
      this.keyboardLook[key]=e.type==='keydown';
      if(this.isLocked)e.preventDefault();
    };
    this._onChange=()=>{const locked=document.pointerLockElement===this.domElement;if(locked!==this.isLocked){this.isLocked=locked;this.dispatchEvent(new Event(locked?'lock':'unlock'));}};
  ...
  updateKeyboardLook(dt){
    ...
    const s=this.keyboardLookSpeed*(window.ZERO_DIVISION_STATE?.settings?.sensitivity||1)*dt;
    const x=(this.keyboardLook.right?1:0)-(this.keyboardLook.left?1:0);
    const y=(this.keyboardLook.down?1:0)-(this.keyboardLook.up?1:0);
    if(x===0&&y===0)return;
    this.yaw-=x*s;
    this.pitch-=y*s;
    const lim=Math.PI/2-.08;
    this.pitch=Math.max(-lim-this.recoilPitch,Math.min(lim-this.recoilPitch,this.pitch));
    this._applyRotation();
  }
